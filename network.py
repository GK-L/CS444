...
class Anchors(nn.Module):
    def __init__(
        ...
        for size in sizes:
            for aspect_ratio in aspect_ratios:
                self.offsets.append([-math.sqrt((size * stride)**2 / aspect_ratio) / 2, -(math.sqrt((size * stride)**2 / aspect_ratio)*aspect_ratio)/2, math.sqrt((size * stride)**2 / aspect_ratio) / 2, (math.sqrt((size * stride)**2 / aspect_ratio)*aspect_ratio)/2])
    ...
    def forward(self, x):
        ...
        """
        sy, sx = torch.meshgrid(torch.arange(x.shape[2], device=x.device) * self.stride, torch.arange(x.shape[3], device=x.device) * self.stride)
        anchors = torch.stack((sx, sy, sx, sy), dim = -1).unsqueeze(2) + self.offsets.to(x.device).unsqueeze(0).unsqueeze(0)
        return anchors.view(x.shape[2], x.shape[3], -1).permute(2, 0, 1).contiguous().unsqueeze(0).repeat(x.shape[0], 1, 1, 1)
