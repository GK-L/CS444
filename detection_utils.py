...
def compute_targets(anchor, cls, bbox):
    ...
    gt_clss = torch.full((anchor.shape[0], anchor.shape[1], 1), 0, device = anchor.device)
    gt_bboxes = torch.full((anchor.shape[0], anchor.shape[1], 4), 0, dtype = torch.float32, device = anchor.device)
    for b in range(anchor.shape[0]):
        ious = compute_bbox_iou(anchor[b], bbox[b])
        gt_clss[b][torch.max(ious, dim = 1)[0] < 0.4] = 0
        gt_clss[b][(torch.max(ious, dim = 1)[0] >= 0.4) & (torch.max(ious, dim = 1)[0] < 0.5)] = -1
        gt_clss[b][torch.max(ious, dim = 1)[0] >= 0.5] = cls[b, torch.max(ious, dim = 1)[1][torch.max(ious, dim = 1)[0] >= 0.5], 0].unsqueeze(-1)
        gt_bboxes[b][torch.max(ious, dim = 1)[0] < 0.5] = 0
        gt_bboxes[b][torch.max(ious, dim = 1)[0] >= 0.5] = bbox[b, torch.max(ious, dim = 1)[1][torch.max(ious, dim = 1)[0] >= 0.5]]
...
def compute_bbox_targets(anchors, gt_bboxes):
    ...
    delta_x = ((gt_bboxes[:, 0] + 0.5 * (gt_bboxes[:, 2] - gt_bboxes[:, 0])) - (anchors[:, 0] + 0.5 * (anchors[:, 2] - anchors[:, 0]))) / (anchors[:, 2] - anchors[:, 0])
    delta_y = ((gt_bboxes[:, 1] + 0.5 * (gt_bboxes[:, 3] - gt_bboxes[:, 1])) - (anchors[:, 1] + 0.5 * (anchors[:, 3] - anchors[:, 1]))) / (anchors[:, 3] - anchors[:, 1])
    delta_w = torch.log(torch.maximum((gt_bboxes[:, 2] - gt_bboxes[:, 0]), torch.tensor(1.0))/(anchors[:, 2] - anchors[:, 0]))
    delta_h = torch.log(torch.maximum((gt_bboxes[:, 3] - gt_bboxes[:, 1]), torch.tensor(1.0))/(anchors[:, 3] - anchors[:, 1]))
...
def apply_bbox_deltas(boxes, deltas):
    ...
    """
    x1 = boxes[:, 0] + 0.5 * (boxes[:, 2] - boxes[:, 0]) + deltas[:, 0] * (boxes[:, 2] - boxes[:, 0])  - 0.5 * torch.exp(deltas[:, 2]) * (boxes[:, 2] - boxes[:, 0]) 
    y1 = boxes[:, 1] + 0.5 * (boxes[:, 3] - boxes[:, 1]) + deltas[:, 1] * (boxes[:, 3] - boxes[:, 1])  - 0.5 * torch.exp(deltas[:, 3]) * (boxes[:, 3] - boxes[:, 1]) 
    x2 = boxes[:, 0] + 0.5 * (boxes[:, 2] - boxes[:, 0]) + deltas[:, 0] * (boxes[:, 2] - boxes[:, 0])  + 0.5 * torch.exp(deltas[:, 2]) * (boxes[:, 2] - boxes[:, 0]) 
    y2 = boxes[:, 1] + 0.5 * (boxes[:, 3] - boxes[:, 1]) + deltas[:, 1] * (boxes[:, 3] - boxes[:, 1])  + 0.5 * torch.exp(deltas[:, 3]) * (boxes[:, 3] - boxes[:, 1])
